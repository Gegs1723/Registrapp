...
export class EscanearPage implements OnInit, ViewWillEnter {
  ...
  async saveAttendance(scan: ScanResult) {
    if (!this.Attendanc.some(r => r.asignatura === scan.asignatura && r.seccion === scan.seccion && r.fecha === scan.fecha)) {
      ...
      this.localStorageService.save(`attendance_${this.username}`, this.Attendanc);
  ...
  ionViewWillEnter(): void {
    this.results = this.localStorageService.get(`attendance_${this.username}`) ?? [];
    this.Attendanc = this.localStorageService.get(`attendance_${this.username}`) ?? [];
  }
